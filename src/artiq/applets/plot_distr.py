...
class MainWidget(QWidget):
    def __init__(self, args):
        ...
        # Tab 3: Data Viewer Controls (Active and default)
        pvt_tab = QWidget()
        ...
        # New: Plot Type dropdown to switch between PvT and Histogram view modes
        self.combo_plot_type = QComboBox()
        self.combo_plot_type.addItems(["Power vs Time", "Power Distribution (Histogram)"])
        dropdown_layout.addRow("Plot View Type:", self.combo_plot_type)
        ...
        # New: Histogram Configuration Group
        hist_group = QGroupBox("Histogram Settings")
        hist_layout = QFormLayout()
        self.hist_scale_edit = QLineEdit("1e9")  # Multiplier factor to convert raw power (e.g. Watts) to nW
        self.hist_auto_bins = QCheckBox("Auto Bins")
        self.hist_auto_bins.setChecked(True)
        self.hist_bins_spin = QSpinBox()
        self.hist_bins_spin.setRange(5, 1000)
        self.hist_bins_spin.setValue(50)
        self.hist_bins_spin.setDisabled(True)
        self.hist_auto_bins.toggled.connect(lambda checked: self.hist_bins_spin.setDisabled(checked))
        
        hist_layout.addRow("Multiplier to nW:", self.hist_scale_edit)
        hist_layout.addRow(self.hist_auto_bins)
        hist_layout.addRow("Bin Count:", self.hist_bins_spin)
        hist_group.setLayout(hist_layout)
        pvt_layout.addWidget(hist_group)
        
        pvt_layout.addStretch()
        self.tabWidget.addTab(pvt_tab, 'Data Viewer')
        ...
        self.combo_plot_type.currentIndexChanged.connect(self.updateActivePlot)
        self.combo_freq.currentIndexChanged.connect(self.updateActivePlot)
        self.combo_att.currentIndexChanged.connect(self.updateActivePlot)
        self.combo_N.currentIndexChanged.connect(self.updateActivePlot)
        self.cb_custom_xlim.stateChanged.connect(self.updateActivePlot)
        self.cb_fix_ylim.stateChanged.connect(self.updateActivePlot)
        self.xlim_min.valueChanged.connect(self.updateActivePlot)
        self.xlim_max.valueChanged.connect(self.updateActivePlot)
        
        # Link new histogram setting updates
        self.hist_scale_edit.textChanged.connect(self.updateActivePlot)
        self.hist_auto_bins.stateChanged.connect(self.updateActivePlot)
        self.hist_bins_spin.valueChanged.connect(self.updateActivePlot)
        
        # Set Data viewer active on startup layout
    ...
    def toggleDarkMode(self, state):
        ...
        for dataset in self.plotWidget.datasets:
            if hasattr(dataset['scatter'], 'setSymbolBrush'):
    ...
    def updateActivePlot(self):
        """Routing dispatcher to switch between Power vs Time and Histogram view."""
        if self.combo_plot_type.currentIndex() == 0:
            self.updatePvTPlot()
        else:
    ...
    def updatePvTPlot(self):
        ...
        if key in self.pvt_data:
            ...
            # Reset default axes labels for time-series mode
            self.plotWidget.setLabel('bottom', getattr(self.args, 'xlabel', 'Time'))
            self.plotWidget.setLabel('left', getattr(self.args, 'ylabel', 'Amplitude'))
    ...
    def updateHistogramPlot(self):
        """Calculates and plots the normalized discrete probability distribution in nW."""
        if not self.pvt_data:
            return
        ...
        plot_power_dist_option = hasattr(self, 'cb_plot_power_dist') and self.cb_plot_power_dist.isChecked()
        
        data_available = False
        y_data = None
        title = ""
        
        if plot_power_dist_option:
            y_data_list = []
            for (f, a, n), data in self.pvt_data.items():
                if f == freq and a == att:
                    y_data_list.append(data.ravel())
            
            if y_data_list:
                y_data = np.concatenate(y_data_list)
                title = f"Power Distribution Histogram: {freq}MHz | {att}dB | All N (Concatenated)"
                data_available = True
        else:
            key = (freq, att, N)
            if key in self.pvt_data:
                ...
                if y_data.ndim > 1:
                    y_data = y_data.ravel()
                title = f"Power Distribution Histogram: {freq}MHz | {att}dB | N={N}"
                data_available = True

        if data_available and y_data is not None:
            # 1. Convert units to nW using user multiplier configuration
            try:
                scale_factor = float(self.hist_scale_edit.text())
            except ValueError:
                scale_factor = 1e9  
                
            y_data_nw = 10 ** (y_data / 10) * 1e-3 * scale_factor
            
            # 2. Extract bin settings
            if plot_power_dist_option:
                if y_data_nw.size > 0:
                    y_min, y_max = np.min(y_data_nw), np.max(y_data_nw)
                    if y_min == y_max:  
                        y_max += 0.01
                    bins_param = np.arange(y_min, y_max + 0.01, 0.01)
                else:
                    bins_param = 'auto'
            else:
                bins_param = 'auto' if self.hist_auto_bins.isChecked() else self.hist_bins_spin.value()
            
            # 3. Calculate histogram counts and scale down heights to probability per bin
            counts, bins = np.histogram(y_data_nw, bins=bins_param)
            total_counts = np.sum(counts)
            probabilities = counts / total_counts if total_counts > 0 else counts
            
            # --- FIX 1: Keep the plot linear to bypass pyqtgraph's log fill bug ---
            self.plotWidget.setLogMode(False, False)
            
            # Safely compute log10 values manually and clip 0-counts to a safe floor
            log_floor = 1e-5
            safe_probabilities = np.clip(probabilities, log_floor, None)
            log_probabilities = np.log10(safe_probabilities)
            
            self.plotWidget.clear_data()
            self.plotWidget.setTitle(title)
            self.plotWidget.setLabel('bottom', 'Power (nW)')
            self.plotWidget.setLabel('left', 'Probability')
            
            # --- FIX 2: Manually format the Y-axis ticks to display log notation strings ---
            ay = self.plotWidget.getAxis('left')
            floor_exp = int(np.floor(np.log10(log_floor)))  # Evaluates to -5
            ticks = [(i, f"{10**i}" if i != 0 else "1") for i in range(floor_exp, 1)]
            ay.setTicks([ticks])
            
            # 4. Generate visual step geometries
            color = self.plotWidget.get_next_color()
            
            # --- FIX 3: Plot pre-logged data array and fill directly down to the numeric floor ---
            bg_item = pyqtgraph.PlotDataItem(
                x=bins,               
                y=log_probabilities, 
                stepMode='center',
                fillLevel=np.log10(log_floor),  # Fills perfectly down to -5 in linear coordinate space
                fillBrush=color, 
                pen=pyqtgraph.mkPen('k', width=0.5)
            )
            self.plotWidget.addItem(bg_item)
            
            # Build internal metadata to retain tracking reference updates
            bin_centers = 0.5 * (bins[:-1] + bins[1:])
            self.plotWidget.datasets.append({
                'x': bin_centers, 
                'y': probabilities, 
                'title': title, 
                'color': color, 
            ...
            # 5. Adapt viewport limits layout
            range_kwargs = {}
            if self.cb_custom_xlim.isChecked():
                range_kwargs['xRange'] = (self.xlim_min.value(), self.xlim_max.value())
            else:
                range_kwargs['xRange'] = (np.min(bins), np.max(bins))
                
            max_prob = max(probabilities) if len(probabilities) > 0 else 1.0
            if max_prob <= 0: 
                max_prob = 1.0
                
            # The scale limits map cleanly directly to our log math constraints
            range_kwargs['yRange'] = (np.log10(log_floor), np.log10(max_prob * 1.1))
            range_kwargs['padding'] = 0.02
            
            self.plotWidget.getViewBox().setRange(**range_kwargs)
        else:
            self.plotWidget.clear_data()
            if plot_power_dist_option:
                self.plotWidget.setTitle(f"No data available for combination {freq}MHz | {att}dB.")
            else:
                self.plotWidget.setTitle(f"Configuration combination ({freq}, {att}, {N}) unavailable.")
...
def main():
    ...
    try:
        ...
        applet.argparser.add_argument('--ylabel', type=str, default='Probability')
        applet.argparser.add_argument('--title', type=str, default='Distribution')
    except Exception:
        pass
    applet.run()
