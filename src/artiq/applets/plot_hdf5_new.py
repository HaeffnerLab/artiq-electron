    ...
    def toggleFit(self, fit_type, update=False):
        ...
        for dataset in self.datasets:
            ...
            if fit_type == 'linear':
                ...
                B0 = (max(xdata_fit) - min(xdata_fit)) / 2
                C0 = min(ydata_fit)
                x0_0 = min(xdata_fit)
                p0 = [A0, B0, C0, x0_0]
                eq_str = 'A exp(-(x-x0)/B) + C'

            elif fit_type == 'exponential increase':
                def func(x, A, B, C, x0):
                    return A * np.exp((x - x0)/B) + C
                A0 = (max(ydata_fit) - min(ydata_fit)) / 10.0
                B0 = (max(xdata_fit) - min(xdata_fit)) / 2.0
                C0 = min(ydata_fit)
                x0_0 = min(xdata_fit)
                p0 = [A0, B0, C0, x0_0]
                eq_str = 'A exp((x-x0)/B) + C'
...
class FitOptionsWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.exp_inc_guess = QLineEdit()  # New field for exponential increase
        ...
        guess_layout.addRow("Exponential decay (A,B,C,x0):", self.exp_guess)
        guess_layout.addRow("Exponential increase (A,B,C,x0):", self.exp_inc_guess)
    ...
    def getOptions(self):
        ...
        exp_inc = parse_guess(self.exp_inc_guess.text())
        ...
        if exp_inc is not None:
            initial_guesses['exponential increase'] = exp_inc
        if dexp is not None:
...
class MainWidget(QWidget):
    def __init__(self, args):
        ...
        self.cb_exp_inc = QCheckBox('Exponential increase')  # Added checkbox
        ...
        fit_layout.addWidget(self.cb_exp_inc)
        ...
        self.cb_exp_inc.stateChanged.connect(lambda state: self.onFitToggled('exponential increase', state))
    ...
    def updateFitCurves(self):
        ...
        if self.cb_exp_inc.isChecked():
            self.plotWidget.toggleFit('exponential increase', update=True)
    ...
    def clearFitOptions(self): 
        ...
        self.cb_exp_inc.setChecked(False)
