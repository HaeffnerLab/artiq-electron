...
# on the data, e.g. the overflows. Here, all bins with overflows are replaced by np.nan.
# These data points will be omitted by pyplot.
data_object = counter.getDataObject()
data_without_overflows = np.where(data_object.getOverflowMask() == 1, np.nan, data_object.getData())

# Plot the result
fig = plt.figure()
plt.plot(data_object.getIndex()/1E12, data_without_overflows[0])
plt.xlabel("Time, s")
plt.ylabel("Counts/s")
data_for_annotation = data_object.getData()[0, :]
annotation_y = np.max(data_for_annotation)
annotation_x = data_object.getIndex()[np.where(data_for_annotation == annotation_y)[0][0]]/1e12
