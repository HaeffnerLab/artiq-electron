...
In this example, we will apply the test signal as an "external clock". Actually, this will
decrease the quality of all measurements and is done solely for demonstration purposes.

In a first experiment, we will investigate the phase error of the software clock. We use a
Correlation measurement to compare the software clock input_signal to the calculated
"ideal_clock_channel". The RMS of the histogram is compared to the instantenous metric
provided by SoftwareClockState.

In a second experiment, we will use the FrequencyStability measurement class to characterize
the test oscillator."""
...
def measure_frequency_stability(tagger, channel, figure_name):
    ...
    # The FrequencyStability measurement takes an argument "steps". These integer values will
...
# The software clock will write its state changes to the logger. For this example,
# we use a custom logger to avoid writing to the console directly. The log messages
...
# Setting up the software clock will require the frequency value of the input signal.
...
With many averaging_periods, the resulting frequency will be more constant,
...
# channel with a perfect period of SoftareClockState.clock_period. It can be interpreted as a grid
