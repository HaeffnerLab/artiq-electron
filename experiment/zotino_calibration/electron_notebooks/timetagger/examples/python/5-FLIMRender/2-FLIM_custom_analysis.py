...
intensity1 = IntensityRenderer(t, start_channel=laser, click_channel=click1, pixel_begin_channel=pixel, pixels_x=xDim,
                               pixels_y=yDim, n_bins=bins, binwidth=binwidth, frame_begin_channel=frame, num_frames=0, divider=1, is_resonant=False)
intensity2 = IntensityRenderer(t, start_channel=laser, click_channel=click2, pixel_begin_channel=pixel, pixels_x=xDim,
                               pixels_y=yDim, n_bins=bins, binwidth=binwidth, frame_begin_channel=frame, num_frames=0, divider=1, is_resonant=False)
...
frames1 = intensity1.getFrames()
frames2 = intensity2.getFrames()
...
for i, frame in enumerate(frames1):
    ...
    fname = 'flim_custom_detector1_%03d.png' % i
...
for i, frame in enumerate(frames2):
    ...
    fname = 'flim_custom_detector2_%03d.png' % i
    plt.savefig(fname, bbox_inches='tight', pad_inches=0)
