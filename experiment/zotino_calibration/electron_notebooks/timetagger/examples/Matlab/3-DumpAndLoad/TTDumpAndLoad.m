...
while file_reader.hasData()
    ...
    eventName = {'0 (TimeTag)', '1 (Error)', '2 (OverflowBegin)', '3 (OverflowEnd)', '4 (MissedEvents)'};
    
    for i = 1:3
        fprintf('    TAG# %8d \t t = %d ps \t Channel: %d \t MissedEvents: %d \t EventType: %s \n', ...
            i, timestamps(i), channel(i), missed_events(i), eventName{overflow_types(i)+1});
    end
    disp('      ...');
    fprintf('    TAG# %8d \t t = %d ps \t Channel: %d \t MissedEvents: %d \t EventType: %s \n\n', ...
            numel(timestamps), timestamps(end), channel(end), missed_events(end), eventName{overflow_types(end)+1});
end
