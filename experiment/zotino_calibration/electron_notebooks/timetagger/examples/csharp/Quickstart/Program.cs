...
namespace SwabianInstruments.TimeTagger.Examples
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // set custom logging
            TT.setLogger((int level, string message) =>
            {
            ...
            WriteColoredLine(ConsoleColor.DarkGreen,
                ...
                "  2) use TimeTagStream;\n" +
                "  3) use a Constum Measurement, which has the best performance for on the fly processing.\n" +
                "\n" +
            ...
            // Example 2
            {
                using SynchronizedMeasurements measurementGroup = new SynchronizedMeasurements(tt);
                ...
                var equal =
                    ...
                    data.Cast<Int64>().SequenceEqual(customData.Cast<Int64>());
                if (equal)
