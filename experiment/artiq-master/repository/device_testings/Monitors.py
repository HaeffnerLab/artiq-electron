...
class Monitors(EnvExperiment):
    def build(self):
        ...
        self.setattr_argument("save_temperature_data", BooleanValue(default=False), group="Data Saving")
        self.setattr_argument("save_power_data", BooleanValue(default=False), group="Data Saving")
        for i in range(2, 8):
            self.setattr_argument(f"enable_extra_channel_{i}", BooleanValue(default=False), group="Extra Channels")
        for i in range(2, 8):
            self.setattr_argument(f"save_extra_channel_{i}_data", BooleanValue(default=False), group="Extra Data Saving")
        

    def prepare(self):
        self.displaying_digits = int(self.displaying_digits)
        self.set_dataset("current_temperature", 0.0, broadcast=True, archive=False)
        self.set_dataset("current_power", 0.0, broadcast=True, archive=False)
        
        if self.save_temperature_data:
        ...
        if self.save_power_data:
            self.set_dataset("power_history", [], broadcast=True, archive=True)

        for i in range(2, 8):
            if getattr(self, f"enable_extra_channel_{i}"):
                self.set_dataset(f"current_channel_{i}_value", 0.0, broadcast=True, archive=False)
                if getattr(self, f"save_extra_channel_{i}_data"):
    ...
    def update_datasets(self, smp, elapsed_time: float):
        temp = smp[0]
        power = smp[1]
        display_temp = round(temp, self.displaying_digits)
        self.set_dataset("current_temperature", display_temp, broadcast=True, archive=False)
        display_power = round(power, self.displaying_digits)
        self.set_dataset("current_power", display_power, broadcast=True, archive=False)
        ...
        for i in range(2, 8):
            if getattr(self, f"enable_extra_channel_{i}"):
                channel_value = smp[i]
                display_value = round(channel_value, self.displaying_digits.int())
