...
import artiq
from Instruments import Valon, SSA3032X_R
...
class ROI_count_full(DAC, pulse_sequence, EnvExperiment):
    ...
    def build(self):
        ...
        self.setattr_argument('RF_drive_freq',NumberValue(default=1.452,unit='GHz',scale=1,ndecimals=3,step=0.001))
        self.setattr_argument('RF_drive_power',NumberValue(default=0,scale=1,ndecimals=3,step=0.001))
    ...
    def run(self):
        ...
        # self.valon = Valon(ip='ASRL/dev/ttyUSB0::INSTR', freq=self.RF_drive_freq*1e9, power=self.RF_drive_power) 
        self.SA = SSA3032X_R(ip='TCPIP::192.168.169.121::INSTR', N_AVG=100, data_trace=3)
        self.load_DAC()
