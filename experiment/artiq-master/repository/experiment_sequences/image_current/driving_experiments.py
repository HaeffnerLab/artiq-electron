...
class LoadingWithDelayedDriving(LoadingExperiments, EnvExperiment):
    ...
    def prepare(self):
        ...
        self.N_driving_freq = len(self.driving_freq.sequence)
        self.N_attenuation = len(self.att.sequence)
    ...
    def run(self):
        ...
        N_data_postdriving = min(int((self.t_data-self.t_driving_delay)/self.SSA_SWT), 1)
        for driving_freq in self.driving_freq.sequence:
            for att in self.att.sequence:
                all_data = []
                for N in range(self.N_repetition):
                    ...
                    run_idx += 1
                self.set_dataset(f'data.{driving_freq/1e6:.0f}MHz.{att:.0f}dB',np.array(all_data),broadcast=True)
