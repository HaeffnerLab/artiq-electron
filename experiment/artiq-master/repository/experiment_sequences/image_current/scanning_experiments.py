...
import time
import numpy as np
from loading_experiments import LoadingExperiments, LoadingWithTwoRFSteps 
...
class LoadingWithRFScans(LoadingWithTwoRFSteps, EnvExperiment):
    """
    Loading with varied RF power stepping sizes
    """
    def build(self):
        LoadingWithTwoRFSteps.build(self)
        self.setattr_argument('sampling_parameters',BooleanValue(default=True), group='Scanning parameters')
        self.setattr_argument('max_scan_points',NumberValue(default=16,unit='',scale=1,ndecimals=0,step=1), group="Scanning parameters")
        self.setattr_argument(
            "P_load_scans", 
    ...
    def prepare(self):
        LoadingWithTwoRFSteps.prepare(self)
        multipole_scan_dict = {'P_load': list(iter(self.P_load_scans)), 
                               'P_detect': list(iter(self.P_detect_scans)), 
                               'delta_P_step': list(iter(self.delta_P_step_scans))}
        self.all_multipole_runs = sample_hyperparameters(multipole_scan_dict, self.max_scan_points, self.sampling_parameters)
        self.N_total = len(self.all_multipole_runs)
        self.mutate_dataset('Progress_index', 1, self.N_total)
    ...
    def run(self):
        ...
        for combo in self.all_multipole_runs: 
            ...
            for m in combo: 
                setattr(self, m, combo[m])
                data_str += f".{m}__{str(round(combo[m],3)).replace('.', 'p')}"
            result = LoadingWithTwoRFSteps.run(self,show_progress=False) 
            self.set_dataset(data_str, result, broadcast=True)
            self.mutate_dataset('Progress_index', 0, progress_idx)
            progress_idx += 1
            if self.recalibrate_V_tip: 
...
class FeedbackWithDCScans(FeedbackExperiments, EnvExperiment):
    """
    Feedback with varied multipoles
    """
    def build(self):
    ...
    def run(self):
        ...
        for combo in self.all_multipole_runs: 
            ...
            for m in combo: 
                setattr(self, m, combo[m])
                data_str += f".{m}__{str(combo[m]).replace('.', 'p')}"
                time_str += f".{m}__{str(combo[m]).replace('.', 'p')}"
            self.load_DAC()
            results, time_stamps = FeedbackExperiments.run(self,show_progress=False) 
            for rep in results: 
                self.set_dataset(f'{data_str}.rep__{rep}',np.array(results[rep]),broadcast=True)
                self.set_dataset(f'{time_str}.rep__{rep}',np.array(time_stamps[rep]),broadcast=True)
            self.mutate_dataset('Progress_index', 0, progress_idx)
            progress_idx += 1
            if self.recalibrate_V_tip: 
                self.calibrate_FET()