...
class Electron(HasEnvironment):
    def setattr_device_nickname(self, device_name, device_nickname=None): 
    ...
    def build(self):
        ...
        self.setattr_device_nickname("ttl2", "ttl_MCP_in")
    ...
    def prepare(self):
        ...
        self.set_dataset(key="optimize.flag.run_mode", value = np.int32(0), broadcast=True, persist=True) # run mode,0: pulse counting, 1: ROI counting (pulse 390), 2: histogram counting (pulse 390)
        
        # parameters: t_load(us),t_wait(us),t_delay(ns), t_acquisition(ns),pulse_counting_time(ms), trigger_level (V), # repetitions, # datapoints
        self.set_dataset(key="optimize.parameter.t_load", value = np.int32(200), broadcast=True, persist=True) # t_load(us)
        self.set_dataset(key="optimize.parameter.t_wait", value = np.int32(100), broadcast=True, persist=True) # t_wait(us)
        self.set_dataset(key="optimize.parameter.t_delay", value = np.int32(450), broadcast=True, persist=True) # t_delay(ns)
        self.set_dataset(key="optimize.parameter.t_acquisition", value = np.int32(600), broadcast=True, persist=True) # t_acquisition(ns)
        self.set_dataset(key="optimize.parameter.pulse_counting_time", value = np.int32(500), broadcast=True, persist=True) # pulse_counting_time(ms)
        self.set_dataset(key="optimize.parameter.trigger_level", value = 0.03, broadcast=True, persist=True) # trigger level (V)
        self.set_dataset(key="optimize.parameter.number_of_repetitions", value = np.int32(1000), broadcast=True, persist=True) # number of repetitions
        self.set_dataset(key="optimize.parameter.number_of_datapoints", value = np.int32(100000), broadcast=True, persist=True) # number of datapoints
        self.set_dataset(key="optimize.parameter.bins", value = np.int32(50), broadcast=True, persist=True) # number of bins in the histogram
        self.set_dataset(key="optimize.parameter.update_cycle", value = np.int32(10), broadcast=True, persist=True) # number of datapoints per update cycle
        self.set_dataset(key="optimize.parameter.V_grid", value = np.int32(10), broadcast=True, persist=True) # number of datapoints per update cycle
        '''
        ...
        # self.pin_matching = {"DC0":9,"DC1":4,"DC2":16,"DC3":6,"DC4":17,"DC5":2,"DC6":14,"DC7":24,"DC8":23,"trigger_level":0} # electrode name : zotino channel
        # self.pin_matching = {
        #     "bl1":19,
        #     "bl2":18,
        #     "bl3":6,
        ...
        self.controlled_electrodes = list(self.pin_matching.keys())
        ...
        self.controlled_electrodes_dict = self.controlled_electrodes
    ...
    def setupUi(self, win):
        self.title = 'Electron GUI 3layer smaller spacer'
        self.left = 0
        self.top = 0
        self.width = 1200 # 600
        self.height = 600 # 200
    ...
    def rolling_run(self):
        ...
        self.update_cycle = np.int32(self.parameter_dict["update_cycle"])
        self.count_tot = 0
        self.count_bins = 0      
    ...
    def get_parameter_dict(self):
        ...
        for p in self.controlled_parameters:
            # parameter_list.append(np.int32(self.get_dataset(key="optimize.parameter."+i)))
...
class MyTabWidget(HasEnvironment,QWidget):
    ...
    def setup_UI(self):
        ...
        self.tl_electrodes = [2,0,1,10]
        self.tr_electrodes = [3,4,5,10]
        ...
        self.tl_electrodes0 = [2,0,1,10]
        self.tr_electrodes0 = [3,4,5,10]
        ...
        grid4.addWidget(hist_button, 15, 7)
        ...
        grid4.addWidget(self.lm_button, 14, 9)
    ...
    def update_multipoles(self):
        ...
        if not self.old_c_file:
            ...
            # if "trigger_level" in self.elec_dict.keys():
            #     voltages = pd.Series(np.zeros(len(self.controlled_electrodes)-1),index = df.index.values)
            # else:
            voltages = pd.Series(np.zeros(len(self.controlled_electrodes)),index = df.index.values)
            ...
            for e in self.elec_dict:
                # print("elec_dic:")
                # print(self.elec_dict)
                self.elec_dict[e] = round(self.elec_dict[e],4)    
            print(self.elec_dict)
            ...
            #self.e is in alphabetical order as in c file: [bl1,...,bl5,br1,...,br5, tg("grid"),tl1,...,tl5,tr1,..,tr5]1':self.e[5],'br2':self.e[6],'br3':self.e[7],'br4':self.e[8],'br5':self.e[9],'tg':self.e[10],'tl1':self.e[11],'tl2':self.e[12],'tl3':self.e[13],'tl4':self.e[14],'tl5':self.e[15],'tr1':self.e[16],'tr2':self.e[17],'tr3':self.e[18],'tr4':self.e[19],'tr5':self.e[20]}
            # print(self.elec_dict)
    ...
    def keyPressEvent(self, qKeyEvent):
        ...
        if qKeyEvent.key() == QtCore.Qt.Key_Return:
            ...
            if self.tabs.currentIndex() == 0:
                ...
                # self.on_run_rigol_extraction_click()
                print(">>> rigol extraction function removed")
        else:
