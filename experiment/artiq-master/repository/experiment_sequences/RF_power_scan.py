...
class RF_power_scan(DAC, pulse_sequence, EnvExperiment):
    ...
    def build(self):
        ...
        self.setattr_argument('t_initial_delay',NumberValue(default=2,unit='s',scale=1,ndecimals=0,step=1)) # delay time at the beginning of the experiment
        ...
        self.setattr_argument("SSA_IP_addr", StringValue(default=f'TCPIP::192.168.169.104::INSTR'))
        self.setattr_argument("RF_Vs_start", NumberValue(default=0.25, unit='V', scale=1, ndecimals=3, step=0.001))
        self.setattr_argument("RF_Vs_stop", NumberValue(default=0.63, unit='V', scale=1, ndecimals=3, step=0.001))
        self.setattr_argument("RF_Vs_step", NumberValue(default=5, unit='mV', scale=1, ndecimals=3, step=0.001))
        # self.setattr_argument('t_interface_delay',NumberValue(default=1,unit='s',scale=1,ndecimals=0,step=1)) # wait time
        self.setattr_device("ttl12")

        self.temp_dir = '/home/electron/artiq/experiment/artiq-master/.temp'
        os.makedirs(self.temp_dir, exist_ok=True)
        

    def prepare(self):
        pulse_sequence.prepare(self)
        self.RF_Vs_step = self.RF_Vs_step * 1e-3
        RF_Vs = np.arange(self.RF_Vs_start, self.RF_Vs_stop+self.RF_Vs_step, self.RF_Vs_step)
        self.number_of_datapoints = len(RF_Vs)
        self.set_dataset('RF_voltage', RF_Vs,broadcast=True) # Number of pulses sent to ttl_MCP_in with ROI in optimize without accumulating
        self.set_dataset('SSA_power',[-2]*self.number_of_datapoints,broadcast=True) # Number of pulses sent to ttl_MCP_in with ROI in optimize without accumulating
        ...
        np.savez(self.temp_dir + f'/RF_temp.npz', t_load=self.t_load, t_wait=self.t_wait, 
                 t_initial_delay=float(self.t_initial_delay), t_acquisition=2, 
                 N_average=float(self.N_SA_avg), RF_Vs_start=float(self.RF_Vs_start), 
                 RF_Vs_stop=float(self.RF_Vs_stop), RF_Vs_step=float(self.RF_Vs_step))
    ...
    def kernel_run_driving_experiment(self):
        ...
        for i in range(self.number_of_datapoints):
            ...
            # delay(self.interface_delay*s) # wait for a while to let the RF power catch up
            if self.continuous_loading:
                self.ttl_390.on()
            count_tot = 0
            i_avg = 0
            power = 0.
            for j in range(10*self.N_SA_avg): ## NOTE: overriding this to hand code the margin
                self.core.break_realtime()
                with sequential:
                    if not self.continuous_loading:
                        self.ttl_390.on()
            ...
            self.mutate_dataset('SSA_power',i,power)
            # self.save_temp(i)
    ...
    #     np.savez(self.temp_dir + f'/RF_temp.npz', RF_index=i)